...
if app_mode == "音频质检":
    ...
    def detect_sudden_noise(file_path, threshold=0.5):
        """
        检测音频中是否存在突发性噪音
        :param file_path: 音频文件路径
        :param threshold: 噪音检测的置信度阈值
        :return: 是否存在突发性噪音（布尔值）
        """
        try:
            # 加载音频
            ...
            # 提取特征
            inputs = processor(y, return_tensors="pt", padding=True, sampling_rate=16000)
            ...
            predicted_label = processor.decode(predicted_ids[0])

            # 检查是否有突发性噪音
            noise_keywords = ["敲击", "犬吠", "爆炸", "尖锐"]  # 根据需要添加关键词
    ...
    def single_round_check(uploaded_files, noise_start=0, noise_end=1000):
        ...
        for uploaded_file in uploaded_files:
            ...
            # 突发性噪音检测
            sudden_noise_status = "✅ 合格" if not detect_sudden_noise(file_path) else "❌ 不合格"
